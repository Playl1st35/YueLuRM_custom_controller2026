...
static Controller_ReFresh_t Controller_ReFresh_data;
int VT_Seq = 0;
...
/************************************************自定义控制器发送的数据*********************************/
void CostomControllerRefresh(referee_id_t *_id, Controller_cmd_s  Controller_Data)
{
	

	uint8_t temp_datalength =  Self_Data_LEN; // 自定义控制器数据长度

	Controller_ReFresh_data.FrameHeader.SOF = REFEREE_SOF;
	Controller_ReFresh_data.FrameHeader.DataLength = temp_datalength;
	Controller_ReFresh_data.FrameHeader.Seq = VT_Seq;
	Controller_ReFresh_data.FrameHeader.CRC8 = Get_CRC8_Check_Sum((uint8_t *)&Controller_ReFresh_data, LEN_CRC8, 0xFF);

	Controller_ReFresh_data.CmdID = ID_self_ctrl;

	Controller_ReFresh_data.Controller_Data = Controller_Data;

	Controller_ReFresh_data.frametail = Get_CRC16_Check_Sum((uint8_t *)&Controller_ReFresh_data, LEN_HEADER + LEN_CMDID + temp_datalength, 0xFFFF);

	RefereeSend((uint8_t *)&Controller_ReFresh_data, LEN_HEADER + LEN_CMDID + temp_datalength + LEN_TAIL); // 发送
	VT_Seq++;                                // 包序号+1
	if (VT_Seq > 255) {
		VT_Seq = 0;
	}
